...
const collectionsQuery = `
  SELECT
    c.*,
    array_agg(task_id) AS tasks, array_agg(submissions_needed) AS submissions_needed
  FROM collections c
  JOIN collections_tasks ct
  ON ct.collection_provider = c.provider AND ct.collection_id = c.id
  WHERE provider = $1
  GROUP BY c.provider, c.id;`
...
const addCollectionsTasksGroupBy = (query) => `
  ...
  JOIN collections_tasks ct
  ON (ct.collection_provider = r.provider AND ct.collection_id = r.collection_id)
  GROUP BY
    r.provider, r.item_id, r.item_data,
...
const itemQuery = `
  ${allItemsQuery}
  WHERE i.provider = $1 AND i.id = $2
  LIMIT 1`

const makeInsertSubmissionQuery = (columns, values) => `
  INSERT INTO submissions (${columns.join(', ')})
    VALUES (${values})
  ON CONFLICT (item_provider, item_id, user_id, task_id, step)
    WHERE NOT skipped
    DO UPDATE SET
